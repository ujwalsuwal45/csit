...
const mathSet1 = [
  ...
  ["Vector has:", ["Magnitude only", "Direction only", "Both magnitude and direction", "None"], 2],
...
const physSet1 = [
  ...
  ["Speed of light in vacuum is:", ["3*10^8 m/s", "3*10^6 m/s", "3*10^5 km/s", "Both A and C are correct"], 3],
  ...
  ["Concave mirror forms:", ["Real image only", "Virtual image only", "Real or virtual depending on distance", "None"], 2],
  ...
  ["A semiconductor has temperature coefficient of resistance which is:", ["Positive", "Negative", "Zero", "Infinite"], 1],
  ...
  ["Power is equal to:", ["Work / time", "Force * distance", "Mass * Acceleration", "None"], 0],
...
const itSet1 = [
  ...
  ["A bug in a program is known as:", ["Error", "Feature", "Glitch", "Both A and C"], 3]
];

function buildSet(setId, title, m, p, c, e, it) {
  let qList = [];
  let qNum = 1;
  const pushSub = (arr, subject) => {
    arr.forEach(itm => {
      qList.push(createQuestion("q"+qNum, subject, itm[0], itm[1], itm[2]));
      qNum++;
    });
  };
  pushSub(m, "Math");
...
// Just duplicate the set slightly shuffled for Set 2 to make 2 large viable mock sets
function shuffle(array) {
  return array.map(v => [...v]).sort(() => Math.random() - 0.5);
}

const sets = [
  buildSet("set1", "HamroCSIT Model Pattern Set 1", mathSet1, physSet1, chemSet1, engSet1, itSet1),
  buildSet("set2", "TU Official Past Year Model Set", shuffle(mathSet1), shuffle(physSet1), shuffle(chemSet1), shuffle(engSet1), shuffle(itSet1))
];
